...
int main(int argc, char** argv){
  ...
  std::string cb_name;
  private_nh.param("cmd_cb_name", cb_name, std::string("cmd_vel"));
  ros::Publisher odom_pub = nh.advertise<nav_msgs::Odometry>("odom", 50);
  ros::Publisher global_vel_pub = nh.advertise<geometry_msgs::Twist>("global_vel", 50);
  ros::Subscriber sub = nh.subscribe(cb_name, 1000, vel_callback);
