...
PathExecutor::~PathExecutor() {
    ...
    nh_local_.deleteParam("robot_type");
    ...
    nh_local_.deleteParam(robot_type_ + "/linear_max_vel");
    nh_local_.deleteParam(robot_type_ + "/linear_acceleration");
    nh_local_.deleteParam(robot_type_ + "/linear_kp");
    nh_local_.deleteParam(robot_type_ + "/linear_brake_distance");

    nh_local_.deleteParam(robot_type_ + "/angular_max_vel");
    nh_local_.deleteParam(robot_type_ + "/angular_min_vel");
    nh_local_.deleteParam(robot_type_ + "/angular_acceleration");
    nh_local_.deleteParam(robot_type_ + "/angular_kp");
    nh_local_.deleteParam(robot_type_ + "/angular_brake_distance");
    nh_local_.deleteParam(robot_type_ + "/angular_margin");

    // spin and dock param
    nh_local_.deleteParam(robot_type_ + "/spin_kp");
    nh_local_.deleteParam(robot_type_ + "/dock_kp");
    nh_local_.deleteParam(robot_type_ + "/spin_and_dock_start_dist");
    nh_local_.deleteParam(robot_type_ + "/straight_margin_rad");
    nh_local_.deleteParam(robot_type_ + "/deviate_margin_rad");
    nh_local_.deleteParam(robot_type_ + "/no_pursuit_y_dis");
    nh_local_.deleteParam(robot_type_ + "/no_pursuit_rad_tolerance");

    // fast spin param
    nh_local_.deleteParam(robot_type_ + "/fast_spin_tolerance");
    nh_local_.deleteParam(robot_type_ + "/fast_spin_kp");
    nh_local_.deleteParam(robot_type_ + "/fast_spin_limit");
...
void PathExecutor::initialize() {
    ...
    fast_spin_enable_ = false;

    t_bef_ = ros::Time::now();
    t_now_ = ros::Time::now();
    odom_bef_ = ros::Time::now();
    
    vel_pub_ = nh_.advertise<geometry_msgs::Twist>("path_exec_cmd_vel", 1);
    ...
    if (pose_type_ == 0) {
        ...
        vel_sub_ = nh_.subscribe("cmd_vel", 50, &PathExecutor::Vel_Callback, this);
    }
    else if (pose_type_ == 1) {
        pose_sub_ = nh_.subscribe("final_pose", 50, &PathExecutor::Pose_Callback, this);
        vel_sub_ = nh_.subscribe("Toposition", 50, &PathExecutor::Vel_Callback, this);
    }
    obst_sub_ = nh_.subscribe("obstacles", 50, &PathExecutor::Obst_Callback, this);
    // odom_sub_ = nh_.subscribe("odom", 50, &PathExecutor::Odom_Callback, this);
    // after_integrate_pub_ = nh_.advertise<geometry_msgs::PoseWithCovarianceStamped>("initialpose", 1);
    ...
    fast_mode_client_ = nh_.serviceClient<std_srvs::SetBool>("fast_spin");
    replan_cnt_ = 0;
    cur_path_idx_ = 0;
    has_start_dacc_x_ = 0;
    has_start_dacc_y_ = 0;
...
bool PathExecutor::initializeParams(std_srvs::Empty::Request& req, std_srvs::Empty::Response& res) {
    ...
    if (robot_type_ != "differential" && robot_type_ != "holonomic") {
        ROS_WARN_STREAM("[Path Exec] " << "robot_type " << robot_type_ << " is invalid");
    }
    nh_local_.param<double>(robot_type_ + "lookahead_d", lookahead_d_, 0.3);
    // linear
    nh_local_.param<double>(robot_type_ + "/linear_max_vel", linear_max_vel_, 0.9);
    nh_local_.param<double>(robot_type_ + "/linear_acceleration", linear_acceleration_, 0.5);
    nh_local_.param<double>(robot_type_ + "/linear_kp", linear_kp_, 2.5);
    nh_local_.param<double>(robot_type_ + "/linear_brake_distance", linear_brake_distance_, 0.3);
    nh_local_.param<double>(robot_type_ + "/linear_margin", linear_margin_, 0.01);
    // nh_local_.param<double>("linear_max_vel", linear_max_vel_, 0.5);

    // angular
    nh_local_.param<double>(robot_type_ + "/angular_max_vel", angular_max_vel_, 0.5);
        nh_local_.param<double>(robot_type_ + "/angular_min_vel", angular_min_vel_, 0.2);
    nh_local_.param<double>(robot_type_ + "/angular_acceleration", angular_acceleration_, 0.2);
    nh_local_.param<double>(robot_type_ + "/angular_kp", angular_kp_, 2.8);
    nh_local_.param<double>(robot_type_ + "/angular_brake_distance", angular_brake_distance_, 0.5);
    nh_local_.param<double>(robot_type_ + "/angular_margin", angular_margin_, 0.01);

    // spin and dock param
    nh_local_.param<double>(robot_type_ + "/spin_kp", spin_kp_, 5.5);
    nh_local_.param<double>(robot_type_ + "/dock_kp", dock_kp_, 1.2);
    nh_local_.param<double>(robot_type_ + "/spin_and_dock_start_dist", spin_and_dock_start_dist_, 0.03);

    // diff drive
    nh_local_.param<double>(robot_type_ + "/straight_margin_rad", straight_margin_rad_, (double)(3.0 * M_PI / 180.0));
    nh_local_.param<double>(robot_type_ + "/deviate_margin_rad", deviate_margin_rad_, (double)(10.0 * M_PI / 180.0));

    // two parameter to fix the rotate when near goal
    // when dis < no_pursuit_y_dis, angle < no_pursuit_rad_tolerance will be ignored
    // and go straight to the nearest point to the goal
    nh_local_.param<double>(robot_type_ + "/no_pursuit_y_dis", no_pursuit_y_dis_, 0.06);
    nh_local_.param<double>(robot_type_ + "/no_pursuit_rad_tolerance", no_pursuit_rad_tolerance_, (double)(30.0 * M_PI / 180.0));

    // fast spin mode
    nh_local_.param<double>(robot_type_ + "/fast_spin_tolerance", fast_spin_tolerance_, 0.03);
    nh_local_.param<double>(robot_type_ + "/fast_spin_kp", fast_spin_kp_, 1.5);
    nh_local_.param<double>(robot_type_ + "/fast_spin_limit", fast_spin_limit_, 2.5);
...
void PathExecutor::Goal_Callback(const geometry_msgs::PoseStamped::ConstPtr& pose_msg) {
    ...
    replan_cnt_ = 0;
    nearest_dis_to_goal_ = cur_pose_.distanceTo(goal_pose_);
    no_pursuit_activate_ = 0;
    ...
    if (robot_type_ == "holonomic") {
        ...
        if (fast_mode_client_.call(fast_spin_srv)) {
            ROS_INFO_STREAM("[Path Executor]: fast spin mode set");
            fast_spin_enable_ = true;
        }
        else {
            fast_spin_enable_ = false;
        }
...
void PathExecutor::Timer_Callback(const ros::TimerEvent& e) {
    ...
    if (working_mode_ == MODE::TRACKING) {
        if (is_XY_Reached(cur_pose_, goal_pose_) && is_Theta_Reached(cur_pose_, goal_pose_) && !is_goal_blocked_) {
            ...
            velocity_state_.x_ = 0;
            velocity_state_.y_ = 0;
            if(robot_type_ == "differential")   velocity_state_.theta_ = 0;
            if(robot_type_ == "holonomic")   velocity_state_.theta_ = Extract_Velocity(VELOCITY::ANGULAR, goal_pose_, rotate_direction_ * angular_acceleration_);
            Velocity_Publish();
            if(robot_type_ == "differential") {
                std_srvs::SetBool fast_spin_srv;
                fast_spin_srv.request.data = true;
                if (!fast_spin_enable_ && fast_mode_client_.call(fast_spin_srv)) {
                    ROS_INFO_STREAM("[Path Executor]: fast spin mode set");
                    fast_spin_enable_ = true;
                }
                else {
                    ROS_WARN_STREAM("[Path Executor]: failed to switch to fast spin");
                    fast_spin_enable_ = false;
                }
                Switch_Mode(MODE::END_SPIN);
            }
            else if(robot_type_ == "holonomic") {
                // double theta_err = std::min(fabs(goal_pose_.theta_ - cur_pose_.theta_), fabs(Angle_Mod(goal_pose_.theta_ - cur_pose_.theta_)));
                // if (theta_err > 0) {
                //     after_fast_spin_mode_ = MODE::TRACKING;
                //     Switch_Mode(MODE::FAST_SPIN);
                // }
                // else {
                    Switch_Mode(MODE::TRACKING);
                // }
        ...
        if (is_Theta_Reached(cur_pose_, goal_pose_)) {
            ...
            if (fast_spin_enable_) {
                std_srvs::SetBool fast_spin_srv;
                fast_spin_srv.request.data = false;
                fast_mode_client_.call(fast_spin_srv);
                fast_spin_enable_ = false;
                ROS_INFO_STREAM("[Path Executor]" << "Switch back to normal localization");
            }
    ...
    else if (working_mode_ == MODE::FAST_SPIN) {
        double theta_err;
        if (after_fast_spin_mode_ == MODE::START_SPIN) {
            theta_err = atan2(goal_pose_.y_ - cur_odom_.y_, goal_pose_.x_ - cur_odom_.x_) - cur_odom_.theta_; 
            if (theta_err > M_PI * 3.0 / 2.0) theta_err = theta_err - 2 * M_PI;
            else if (theta_err < - M_PI * 3.0 / 2.0) theta_err = theta_err + 2 * M_PI;

            if (theta_err > M_PI / 2.0) {
                theta_err = theta_err - M_PI;
            }
            else if (theta_err < -M_PI / 2.0) {
                theta_err = M_PI + theta_err;
            }
        }
        else {
            theta_err = goal_pose_.theta_ - cur_odom_.theta_;
        }
        if (theta_err > M_PI) theta_err = theta_err - 2 * M_PI;
        if (theta_err < -M_PI) theta_err = 2 * M_PI + theta_err; 
        if (fabs(theta_err) < fast_spin_tolerance_) {
            velocity_state_.x_ = 0;
            velocity_state_.y_ = 0;
            velocity_state_.theta_ = 0;
            Velocity_Publish();

            geometry_msgs::PoseWithCovarianceStamped rough_pose;
            rough_pose.pose.pose.position.x = cur_odom_.x_;
            rough_pose.pose.pose.position.y = cur_odom_.y_;
            tf2::Quaternion myQ;
            myQ.setRPY(0, 0, cur_odom_.theta_);
            rough_pose.pose.pose.orientation.x = myQ.getX();
            rough_pose.pose.pose.orientation.y = myQ.getY();
            rough_pose.pose.pose.orientation.z = myQ.getZ();
            rough_pose.pose.pose.orientation.w = myQ.getW();
            // after_integrate_pub_.publish(rough_pose);

            Switch_Mode(after_fast_spin_mode_);
            return ;
        }
        double angle_vel = theta_err * fast_spin_kp_;

        if (angle_vel > fast_spin_limit_) angle_vel = fast_spin_limit_;
        if (angle_vel < -fast_spin_limit_) angle_vel = -fast_spin_limit_;
        velocity_state_.x_ = 0;
        velocity_state_.y_ = 0;
        velocity_state_.theta_ = angle_vel;
        Velocity_Publish();
    }
    else {
...
void PathExecutor::Switch_Mode(MODE next_mode) {
    ...
    if (working_mode_ == MODE::IDLE) {
        ...
        ROS_INFO_STREAM("[Path Executor]: switch mode to fast spin");
    }
    else {
        ROS_ERROR("[Path Executor]: WRONG MODE!");
    }
...
bool PathExecutor::is_XY_Reached(RobotState cur_pos, RobotState goal_pos) {
    if (robot_type_ == "differential") {
        double cur_dis = cur_pos.distanceTo(goal_pos);
        // ROS_INFO_STREAM("[Path Executor]:" << __LINE__ << " xy_tolerance " << cur_dis << " " << nearest_dis_to_goal_);

        // a*b=|a||b|*cos(theta) ==> theta = acos(a*b/|a||b|)
        double angle_with_goal = acos((cos(cur_pos.theta_)*(goal_pos.x_ - cur_pos.x_)+sin(cur_pos.theta_)*(goal_pos.y_ - cur_pos.y_))/(sqrt(pow(goal_pos.x_-cur_pos.x_,2)+pow(goal_pos.y_-cur_pos.y_,2))));
        if (angle_with_goal > M_PI / 2.0) {
            angle_with_goal = M_PI - angle_with_goal;
        }

        if (cur_dis*cos(angle_with_goal) < xy_tolerance_) {
            XY_reached_cnt_ ++;
        }
        else {
            nearest_dis_to_goal_ = std::min(nearest_dis_to_goal_, cur_dis);
            XY_reached_cnt_ = 0;
        }
        // ROS_INFO("[Path Executor] -> XY_reached_cnt : %d", XY_reached_cnt_);
        if(XY_reached_cnt_ >= 4){
            return true;
        }
        else    return false;
    }
        ...
        // ROS_INFO("[Path Executor] -> XY_reached_cnt : %d", XY_reached_cnt_);
        if(XY_reached_cnt_ >= 6){
            return true;
        }
        else    return false;
    }
    else {
        ROS_INFO_STREAM("[Path Executor]" << " Wrong robot_type!");
        return true;
    }
}
...
bool PathExecutor::is_Theta_Reached(RobotState cur_pos, RobotState goal_pos) {
    double theta_err = 0;
    double theta_debounce_thershold = 0;
    ...
    // ROS_INFO("[Path Executor] -> Theta_reached_cnt : %d", Theta_reached_cnt_);
    
    if(robot_type_ == "differential")   theta_debounce_thershold = 3;
    if(robot_type_ == "holonomic")   theta_debounce_thershold = 6;

    if(Theta_reached_cnt_ >= theta_debounce_thershold){
        return true;
    }
    else    return false;
}
...
// void PathExecutor::Odom_Callback(const nav_msgs::Odometry::ConstPtr& pose_msg) {
//     cur_odom_.x_ = pose_msg->pose.pose.position.x;
//     cur_odom_.y_ = pose_msg->pose.pose.position.y; 
//     tf2::Quaternion q;
//     tf2::fromMsg(pose_msg->pose.pose.orientation, q);
//     tf2::Matrix3x3 qt(q);
//     double _, yaw;
//     qt.getRPY(_, _, yaw);
//     cur_odom_.theta_ = yaw;
// }

void PathExecutor::Vel_Callback(const geometry_msgs::Twist::ConstPtr& vel_msg) {
    if (working_mode_ == MODE::FAST_SPIN) {
        double dt = (ros::Time::now() - odom_bef_).toSec();
        cur_odom_.x_ += vel_msg->linear.x * dt;
        cur_odom_.y_ += vel_msg->linear.y * dt;
        cur_odom_.theta_ += vel_msg->angular.z * dt;
    }
    else {
        cur_odom_.x_ = cur_pose_.x_;
        cur_odom_.y_ = cur_pose_.y_;
        cur_odom_.theta_ = cur_pose_.theta_;
    }
    odom_bef_ = ros::Time::now();
}
...
void PathExecutor::Omni_Controller(RobotState local_goal) {
    ...
    if (working_mode_ == MODE::IDLE) {
        ...
        if (cur_vec.cross(goal_vec)(2) >= 0)    rotate_direction = 1;
...
void PathExecutor::Diff_Controller(RobotState local_goal) {
    ...
    // } 
        ...
        if (linear_velocity * velocity_state_.x_ < 0 && cur_pose_.distanceTo(goal_pose_) < 0.2) {
            // Reverse direction too quickly, might be dangerous
            ROS_WARN_STREAM("[Path Executor] : Reverse direction too quickly, might be dangerous");
            is_goal_blocked_ = true; 
            return ;
        }
        if (keep_x == 1) {
            ROS_WARN_STREAM("[Path Executor]" << " keep_x");
        }
        if(!keep_x){
            // ROS_INFO_STREAM("[Path Executor]" << "@" << no_pursuit_y_dis_ << " " << no_pursuit_rad_tolerance_);
            if (no_pursuit_activate_ || (cur_pose_.distanceTo(goal_pose_) < no_pursuit_y_dis_ && std::fabs(relative_steer_angle_) < no_pursuit_rad_tolerance_)) {
                // ROS_INFO_STREAM("[Path Executor]: " << "Angle under threshold, won't pursuit y diff from now on.");
                angular_velocity = 0;
                no_pursuit_activate_ = 1;
            }
            else if (relative_radius_ == -1) {
                angular_velocity = 0;
            }
            else {
                // ROS_WARN_STREAM("[Path Executor]" << "this line should not have been seen after activae " << relative_steer_angle_);
        ...
        diff_radius.data = no_pursuit_activate_;
...
double PathExecutor::Extract_Velocity(VELOCITY vel_type, RobotState goal_pos, double acceleration) {
    ...
    if (working_mode_ == MODE::TRACKING) {
        if (vel_type == VELOCITY::LINEAR) {
            ...
            if (robot_type_ == "holonomic"){
            //     if (output_vel > std::min(angular_max_vel_, last_vel + d_vel)) {
            //         output_vel = std::min(angular_max_vel_, last_vel + d_vel);
            //     }
...
void PathExecutor::SpinAndDock(RobotState local_goal, bool opt) {
    ...
    double time_passed = (ros::Time::now() - t_bef_).toSec();
    double cur_vel_lim = (fast_spin_enable_ ? fast_spin_limit_ : angular_max_vel_);
    if (opt == 0) {
        CalculateSteer(local_goal);
        ...
        if (relative_steer_angle_ > deviate_margin_rad_) {
            ...
            if (angular_velocity > cur_vel_lim) angular_velocity = cur_vel_lim;
            if (angular_velocity < -cur_vel_lim) angular_velocity = -cur_vel_lim;

            if (angular_velocity > 0) angular_velocity = std::min(angular_velocity, velocity_state_.theta_ + angular_acceleration_ * time_passed);
            if (angular_velocity < 0) angular_velocity = std::max(angular_velocity, velocity_state_.theta_ - angular_acceleration_ * time_passed);


            if (angular_velocity > 0 && angular_velocity < angular_min_vel_) angular_velocity = angular_min_vel_;
            if (angular_velocity < 0 && angular_velocity > -angular_min_vel_) angular_velocity = - angular_min_vel_;
            velocity_state_.x_ = 0;
            velocity_state_.theta_ = angular_velocity;
            Velocity_Publish();
            ...
            fast_mode_client_.call(fast_spin_srv);
            fast_spin_enable_ = false;
            ROS_INFO_STREAM("[Path Executor]" << "Switch to fast spin in SpinAndDock");
        ...
        if (is_Theta_Reached(cur_pose_, local_goal)) {
            velocity_state_.x_ = 0;
            velocity_state_.y_ = 0;
            velocity_state_.theta_ = 0;
            Velocity_Publish();
            Switch_Mode(MODE::IDLE);
            ROS_INFO_STREAM("[Path Executor]: SpinAndDock finished");
            is_goal_blocked_ = false;
